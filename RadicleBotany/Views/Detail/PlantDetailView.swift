    ...
    var body: some View {
        NavigationStack {
            VStack(spacing: 24) {
                ...
                Text("This feature requires a Lifetime or Pro subscription.")
                    .font(AppFont.body())
                    .foregroundStyle(Color.textSecondary)
                ...
                VStack(spacing: 12) {
                    if let lifetime = storeManager.lifetimeProduct {
                        Button {
                            Task { _ = await storeManager.purchase(lifetime) }
                        } label: {
                    ...
                    if let proYearly = storeManager.proYearlyProduct {
                        Button {
                            Task { _ = await storeManager.purchase(proYearly) }
                        } label: {
                            Text("Pro Yearly - \(proYearly.displayPrice)")
                        }
